...
//       const response = await axios.post('/api/codelabs/create', payload); // Replace with actual API endpoint
...
import React, { useState } from 'react';
import { useAuth } from '../context/AuthContext'; // Assuming you're using some auth context
import { createCodelab } from '../../../server/Routes/'; // Axios route to backend
import { useNavigate } from 'react-router-dom';
...
export default function MyCodeLabs() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Call backend route to create codelab
      const response = await createCodelab(payload);
